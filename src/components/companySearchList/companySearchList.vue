  ...
  <div>
    <ul class="my_company_list">
      <li v-for="item in companyList">
        <img :src="item.img?path+item.img:''">
        <section>
          <h1>
            <a>
            ...
            <span>
              <span class="auditing">{{item.enterprise?item.enterprise.claim_status:''}}</span>
            </span>
          </h1>
          <p>
            <span class="icon-tel"></span>
            <span>电话:{{item?item.phone:''}}</span>
            <span class="icon-message"></span>
            <span>邮箱:{{item?item.email:''}}</span>
            <span class="icon-globe"></span>
            <span>网址:<a :href="item?item.web_url:''">{{item?item.web_url:''}}</a></span>
          </p>
          <p>
            <span class="icon-compass"></span>
            <span>地址:{{item?item.address:''}}</span>
          </p>
          <p>
            <router-link :to="{path:userId?'/manage/personal/myCompany/myCompanyClaimApply':'/login',query:{name:item.title,type:'company',id:item.id}}"><span class="icon-id"></span>认领公司</router-link>
          </p>
        </section>
      </li>
      <!-- <li>
